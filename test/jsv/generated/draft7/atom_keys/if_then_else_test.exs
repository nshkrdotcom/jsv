...
defmodule JSV.Generated.Draft7.AtomKeys.IfThenElseTest do
  alias JSV.Test.JsonSchemaSuite
  use ExUnit.Case, async: true

  @moduledoc """
  Test generated from deps/json_schema_test_suite/tests/draft7/if-then-else.json
  """

  describe "ignore if without then or else" do
    setup do
      json_schema = %JSV.Schema{if: %{const: 0}}
  ...
  describe "ignore then without if" do
    setup do
      json_schema = %JSV.Schema{then: %{const: 0}}
      schema = JsonSchemaSuite.build_schema(json_schema, default_meta: "http://json-schema.org/draft-07/schema")
      {:ok, json_schema: json_schema, schema: schema}
    end
  ...
  describe "ignore else without if" do
    setup do
      json_schema = %JSV.Schema{else: %{const: 0}}
      schema = JsonSchemaSuite.build_schema(json_schema, default_meta: "http://json-schema.org/draft-07/schema")
      {:ok, json_schema: json_schema, schema: schema}
    end
  ...
  describe "if with boolean schema true" do
    setup do
      json_schema = %JSV.Schema{else: %{const: "else"}, if: true, then: %{const: "then"}}
      schema = JsonSchemaSuite.build_schema(json_schema, default_meta: "http://json-schema.org/draft-07/schema")
      {:ok, json_schema: json_schema, schema: schema}
    end
  ...
  describe "if with boolean schema false" do
    setup do
      json_schema = %JSV.Schema{else: %{const: "else"}, if: false, then: %{const: "then"}}
      schema = JsonSchemaSuite.build_schema(json_schema, default_meta: "http://json-schema.org/draft-07/schema")
      {:ok, json_schema: json_schema, schema: schema}
    end
